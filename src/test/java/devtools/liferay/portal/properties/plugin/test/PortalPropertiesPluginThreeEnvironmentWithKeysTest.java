 ...
 * }
 */
public class PortalPropertiesPluginThreeEnvironmentWithKeysTest {
    ...
    private void createFolders(TemporaryFolder temporaryFolder) throws IOException {
        ...
        InputStream inputStreamUatKeysFile = new StringInputStream(KEYS_CONTENT);
        InputStream inputStreamProdKeysFile = new StringInputStream(KEYS_CONTENT);
    ...
    private boolean checkEmptyFolders(TemporaryFolder temporaryFolder) throws IOException{
        return temporaryFolder.getRoot().exists() && temporaryFolder.getRoot().isDirectory() &&
                ...
                FileUtils.readFileToString(Paths.get(temporaryFolder.getRoot().getAbsolutePath() + "/" + DESTINATION_FOLDER_NAME + "/" + UAT_ENVIRONMENT_NAME + "/" + PORTAL_PROPERTIES_FILE_NAME + "." + PROPERTIES_FILE_EXTENSION).toFile()).equals(PORTAL_PROPERTIES_DEST) &&
                FileUtils.readFileToString(Paths.get(temporaryFolder.getRoot().getAbsolutePath() + "/" + DESTINATION_FOLDER_NAME + "/" + PROD_ENVIRONMENT_NAME + "/" + PORTAL_PROPERTIES_FILE_NAME + "." + PROPERTIES_FILE_EXTENSION).toFile()).equals(PORTAL_PROPERTIES_DEST)
                ;
    }
}
